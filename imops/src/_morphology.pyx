...
cdef inline int filter_iterator_offset(filter_iterator *filter_iter, int *coordinates, np.uint8_t is_dilation) nogil:
    cdef int i, offset = 0

    for i in range(3):
        if coordinates[i] < filter_iter[0].bound1[i]:
            offset += filter_iter[0].strides[i] * coordinates[i]
        elif coordinates[i] >= filter_iter[0].bound2[i]:
            offset += filter_iter[0].strides[i] * (filter_iter[0].bound1[i] + coordinates[i] - filter_iter[0].bound2[i])
        else:
            offset += filter_iter[0].strides[i] * filter_iter[0].bound1[i]
...
cdef inline int worker(
    ...
    cdef int *oo = offsets + filter_iterator_offset(&filter_iter, <int*>input_iter.coordinates, is_dilation)
...
def _binary_operation(
    ...
    cdef np.uint8_t[:, :, ::1] output = np.zeros_like(input, dtype=np.uint8)
    ...
    for task in prange(num_threads, nogil=True, num_threads=num_threads):
        ...
        worker(
            &c_input[0, 0, 0], &c_footprint[0, 0, 0], &output[0, 0, 0],
...
def new_binary_erosion(np.uint8_t[:, :, :] input, np.uint8_t[:, :, :] footprint, Py_ssize_t num_threads):
    return _binary_operation(input, footprint, num_threads, True, False)


def new_binary_dilation(np.uint8_t[:, :, :] input, np.uint8_t[:, :, :] footprint, Py_ssize_t num_threads):
