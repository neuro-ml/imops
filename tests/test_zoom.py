...
def test_zoom_to_shape():
    for i in range(16):
        shape = np.random.randint(256, 512, size=np.random.randint(1, 4))
        new_shape = np.random.poisson(shape)
        inp = np.random.randn(*shape)
        axis = np.arange(len(inp.shape))

        without_borders = np.index_exp[:-1, :-1, :-1][: inp.ndim]

        # rtol=1e-6 as there is still some inconsistence
        allclose(
...
def test_identity(zooms):
    test_zoom, _ = zooms
    for i in range(16):
        shape = np.random.randint(2, 256, size=np.random.randint(1, 4))
        inp = np.random.randn(*shape)
...
def test_scale_types(zooms):
    ...
    inp = np.random.randn(128, 128, 128)
...
def test_thin(zooms):
    ...
    for i in range(3):
        for j in range(16):
            shape = [1 if k < i else np.random.randint(2, 256) for k in range(3)]
            inp = np.random.randn(*shape)
